...
The same artifact type can be accessed through different access specifications. For example, an `ociImage` could be stored in an OCI registry (`OCIImage`) or bundled into a CTF archive (`localBlob`).
