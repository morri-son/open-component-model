...
flowchart TD
    ...
    Promote --> PubCompFinal[publish_components_final<br/>ocm.software/cli, /controller, /ocm<br/>conflict: abort-and-fail, :latest if applicable]
...
Every release publishes three OCM component-versions to `ghcr.io/<owner>/component-descriptors/`, all carrying the same version as the GitHub release. RC publications use the `replace` conflict policy so rerunning a failed job is idempotent; final publications use `abort-and-fail` because final component-versions are immutable.
