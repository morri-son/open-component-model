...
SPEAKER_NOTES: dict[int, str] = {
    1: (
        "Open with the observation, not the product.\n"
    ...
    2: (
        ...
        "Calibration for cosign shops: keep signing each piece. What is missing is a name for the release as one unit, signable, verifiable in a sovereign zone with no callback.\n"
    ...
    3: (
        ...
        "Promote EU → US → air-gapped CTF: coordinate stays, digest stays, only access is rewritten. Signature still verifies — anywhere.\n"
        "Move the artifact. The digest stays. Only the access changes. That is the whole trick."
    ),
    4: (
        "Two jobs: pre-empt 'what does this replace?' and define the noun the rest of the deck rests on.\n"
    ...
    10: (
        ...
        "Same command in all three: `ocm transfer cv <src> <dst>` or `ocm transfer ctf <src> <dst>`. Access changes; digests do not. Signature covers digests, so it survives every hop. Verification is purely local at the destination — that is the air-gap property."
    ),
    11: (
