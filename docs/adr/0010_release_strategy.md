...
**Conflict policy is split by phase:**

* **RC** uses `--component-version-conflict-policy replace`. RC tags are themselves mutable (RC2 supersedes RC1), and reruns after a partial failure must be idempotent.
* **Final** uses `--component-version-conflict-policy abort-and-fail`. Final component-versions are immutable; a conflict at this point indicates that an earlier final run partially succeeded and operator intervention is required.
